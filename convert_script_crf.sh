...
if [[ -f ${FILENAME}.${FMT} ]]; then rm ${FILENAME}.${FMT}; fi
echo 'CRF:' $CRF; echo 'SCALE:' $SCALE; echo 'FILENAME:' $FILENAME.$EXTENSION; echo 'FMT:' $FMT; sleep 5
echo
ffmpeg -i ${FILENAME}.$EXTENSION -c:v libvpx-vp9 -b:v 0 -crf $CRF -pass 1 -an -f null /dev/null && \
ffmpeg -i ${FILENAME}.$EXTENSION -c:v libvpx-vp9 -b:v 0 -crf $CRF -pass 2 -an -vf scale=$SCALE ${FILENAME}_crf.${FMT}
